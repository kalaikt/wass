...
/* HC Tree */
function toggleTreeHC(expandableElement, hcId, ttId) {
	if (!jQuery.isPlainObject(__hcTree))
		return false;
	var pCat = __hcTree[hcId];
	if (!jQuery.isPlainObject(pCat) || ( !jQuery.isPlainObject(pCat['children'])) || (jQuery.isEmptyObject(pCat['children'])))
		return false;
	...
	var innerHtml = '';
	for (var cId in pCat['children']) {
		var cat = __hcTree[cId];
		if (!jQuery.isPlainObject(cat))
			continue;
		var isExpandable = !jQuery.isEmptyObject(cat['children']);
		innerHtml += '<li>'
			+(isExpandable ? '<div onClick="toggleTreeHC(this, '+cat['id']+','+ttId+');"></div>' : '')
            +'<span><a href="'+cat['url']+'">'+cat['category']+'</a></span>'
			+(isExpandable ? '<ul></ul>' : '')
			+'</li>';
	}
